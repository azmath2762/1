...
def error_code(error_picklist):
    ...
    new_order_id_list = [-1]
    completed_file_path=r'C:\Users\Administrator\PycharmProjects\pythonProject\flipkart_processing_code\completed_records\completed'+error_picklist+'.csv'
    failed_file_path=r'C:\Users\Administrator\PycharmProjects\pythonProject\flipkart_processing_code\failed_records\failed'+error_picklist+'.csv'
    with open(failed_file_path, "a", newline='') as f:
        thewriter = csv.writer(f)
        thewriter.writerow(['listingId', 'picklistId_mongo', 'warehouseId_mongo','bin_id','error massage'])
    with open(completed_file_path, "a", newline='') as f:
        thewriter = csv.writer(f)
    ...
    file=r'C:\Users\Administrator\PycharmProjects\pythonProject\flipkart_processing_code\failed_records\failed'+error_picklist+'.csv'
    with open(file, "r", newline='')as f1:
        ...
        for i in range(1,len(r)):
            ...
            try:
                ...
                try:
                    ...
                    for i in range(1, 20):
                        try:
                            listing2 = driver.find_element_by_xpath(
                                '(//div[@class="product-listing-id clearfix"])[' + str(i) + ']')
                ...
                with open(failed_file_path, "a", newline='') as f:
                    ...
                    time.sleep(1)
                    continue
    dropbox_failed_file(failed_file_path,error_picklist)
    dropbox_completed_file(completed_file_path,error_picklist)
error_code('P201021-FFB1B341E017')
        # except Exception as e:
        #     print('total error:',e)
