...
def updating_in_error(id):
    ...
    picklist_conn = psycopg2.connect(host=rds_host, database=db_name, user=name, password=password)
    picklist_cur = picklist_conn.cursor()
    statement = "update original_picklist set is_processing=false where status=false and id="+str(id)+""
    print(statement)
    picklist_cur.execute(statement)
...
def get_details_from_source_table():
    rds_host = "buymore-2.cluster-cegnfd8ehfoc.ap-south-1.rds.amazonaws.com"
    db_name = 'warehouse'
    name = 'postgres'
    password = 'buymore3'
    ...
    picklist_conn = psycopg2.connect(host=rds_host, database=db_name, user=name, password=password)
    picklist_cur = picklist_conn.cursor()
    statement = "SELECT picklist_id,order_picklist_listing_id,warehouse_id,id FROM public.original_picklist where status!=true and is_processing!=true and warehouse_id=10 and portal_id=2 and picklist_id='P201021-C04C9AABF37D'"
    picklist_cur.execute(statement)
...
def flipkart_processing_main_code():
    ...
    global list1
    picklist_data = get_details_from_source_table()
    ...
    for i in range(0, len(pick_data)+10):
        try:
            ...
            print('warehouseId_mongo',warehouseId_mongo)
